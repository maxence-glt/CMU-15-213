...
0000000000400da0 <main>:
  ...
  400e6f:	48 89 c7             	mov    %rax,%rdi	//arg1 = read_line
...
0000000000400efc <phase_2>:
  ...
  400f0a:	83 3c 24 01          	cmpl   $0x1,(%rsp)              // cmp (*rsp - 1)       rsp must == 1
  400f0e:	74 20                	je     400f30 <phase_2+0x34>    // eq? jmp to 0x400f30
  400f10:	e8 25 05 00 00       	callq  40143a <explode_bomb>
  400f15:	eb 19                	jmp    400f30 <phase_2+0x34>
  400f17:	8b 43 fc             	mov    -0x4(%rbx),%eax          // jmp from 400f3a to here, mov *(rbx - 4) to eax
  400f1a:	01 c0                	add    %eax,%eax                // eax * 2
  400f1c:	39 03                	cmp    %eax,(%rbx)              // cmp (*rbx - eax) 
  400f1e:	74 05                	je     400f25 <phase_2+0x29>    // eq? jmp to 0x400f25
  400f20:	e8 15 05 00 00       	callq  40143a <explode_bomb>
  400f25:	48 83 c3 04          	add    $0x4,%rbx                // add 4 to rbx
  400f29:	48 39 eb             	cmp    %rbp,%rbx                // cmp (rbx - rbp)
  400f2c:	75 e9                	jne    400f17 <phase_2+0x1b>    // 
  400f2e:	eb 0c                	jmp    400f3c <phase_2+0x40>    // eq? jmp to 0x400f3c
  400f30:	48 8d 5c 24 04       	lea    0x4(%rsp),%rbx          // jmp from 400f0e to here, lea rsp+4 -> rbx
  400f35:	48 8d 6c 24 18       	lea    0x18(%rsp),%rbp         // lea rsp + 24 -> rbp
  400f3a:	eb db                	jmp    400f17 <phase_2+0x1b>   // jmp to 0x400f17
  400f3c:	48 83 c4 28          	add    $0x28,%rsp              // add 40 bytes to stack
  400f40:	5b                   	pop    %rbx
  400f41:	5d                   	pop    %rbp
  400f42:	c3                   	retq   

0000000000400f43 <phase_3>:
  400f43:	48 83 ec 18          	sub    $0x18,%rsp	// increase stack by 18
  400f47:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx	// arg4 = stack + 12
  400f4c:	48 8d 54 24 08       	lea    0x8(%rsp),%rdx	// arg3 = stack + 8
  400f51:	be cf 25 40 00       	mov    $0x4025cf,%esi	// arg2 = %d %d
  400f56:	b8 00 00 00 00       	mov    $0x0,%eax	// return = 0x0
  400f5b:	e8 90 fc ff ff       	callq  400bf0 <__isoc99_sscanf@plt>
  400f60:	83 f8 01             	cmp    $0x1,%eax	// check if theres more than one input
  400f63:	7f 05                	jg     400f6a <phase_3+0x27> 
  400f65:	e8 d0 04 00 00       	callq  40143a <explode_bomb>
  400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp)   //  rsp + 8 - 0x7
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a> // explode if rsp + 8 > 0x7
  400f71:	8b 44 24 08          	mov    0x8(%rsp),%eax	// eax = first input
  400f75:	ff 24 c5 70 24 40 00 	jmpq   *0x402470(,%rax,8) // jump to memory at 0x402470 + firstInput * 8
  // use x/16gx 0x402470
  // 0x402470:	0x0000000000400f7c	 i = 0, jump to right after this
  // 0x402480:	0x0000000000400f83	 i = 1, jump to 400f83
  // 0x402490:	0x0000000000400f91	 i = 2, jump to 400f91
  // 0x4024a0:	0x0000000000400f9f	 i = 3, jump to 400f9f
  // 0x4024b0 <array.3449>:	0x737265697564616d	
  // 0x4024c0:	0x7420756f79206f53	
  // 0x4024d0:	0x6f7473206e616320	
  // 0x4024e0:	0x206874697720626d	
  400f7c:	b8 cf 00 00 00       	mov    $0xcf,%eax   // eax = 207
  400f81:	eb 3b                	jmp    400fbe <phase_3+0x7b>    // jmp to 400fbe
  400f83:	b8 c3 02 00 00       	mov    $0x2c3,%eax  // eax = 707
  400f88:	eb 34                	jmp    400fbe <phase_3+0x7b>    // jmp to 400fbe
  400f8a:	b8 00 01 00 00       	mov    $0x100,%eax  // 
  400f8f:	eb 2d                	jmp    400fbe <phase_3+0x7b>    // jmp to 400fbe
  400f91:	b8 85 01 00 00       	mov    $0x185,%eax  // eax = 389
  400f96:	eb 26                	jmp    400fbe <phase_3+0x7b>    // jmp to 400fbe
  400f98:	b8 ce 00 00 00       	mov    $0xce,%eax
  400f9d:	eb 1f                	jmp    400fbe <phase_3+0x7b>    // jmp to 400fbe
  400f9f:	b8 aa 02 00 00       	mov    $0x2aa,%eax  // eax = 682
  400fa4:	eb 18                	jmp    400fbe <phase_3+0x7b>    // jmp to 400fbe
  400fa6:	b8 47 01 00 00       	mov    $0x147,%eax
  400fab:	eb 11                	jmp    400fbe <phase_3+0x7b>    // jmp to 400fbe
  400fad:	e8 88 04 00 00       	callq  40143a <explode_bomb>	// explode
  400fb2:	b8 00 00 00 00       	mov    $0x0,%eax
  400fb7:	eb 05                	jmp    400fbe <phase_3+0x7b>
  400fb9:	b8 37 01 00 00       	mov    $0x137,%eax
  400fbe:	3b 44 24 0c          	cmp    0xc(%rsp),%eax		// 400fbe, check if j = eax (1 = 0, eax = 207; i = 1, eax = 707; i = 2, eax = 389, i = 3, eax = 682)
  400fc2:	74 05                	je     400fc9 <phase_3+0x86>
  400fc4:	e8 71 04 00 00       	callq  40143a <explode_bomb>	// explode
  400fc9:	48 83 c4 18          	add    $0x18,%rsp
  400fcd:	c3                   	retq   

0000000000400fce <func4>: // rdi is i, rsi is 0, rdx is 14
  400fce:	48 83 ec 08          	sub    $0x8,%rsp      // add 1 byte to stack
  400fd2:	89 d0                	mov    %edx,%eax      // move 14 to return 
  400fd4:	29 f0                	sub    %esi,%eax      // 14 - 0
  400fd6:	89 c1                	mov    %eax,%ecx      // move 14 to rcx
  400fd8:	c1 e9 1f             	shr    $0x1f,%ecx     // rcx = rcx >> 31 (rcx = 0)
  400fdb:	01 c8                	add    %ecx,%eax      // return += rcx (return + 0)
  400fdd:	d1 f8                	sar    %eax	      // return >> 1 (return = 7)
  400fdf:	8d 0c 30             	lea    (%rax,%rsi,1),%ecx   // rcx = 7
  400fe2:	39 f9                	cmp    %edi,%ecx      // rcx - edi
  400fe4:	7e 0c                	jle    400ff2 <func4+0x24>  // jmp  to A if rcx <= edi
  400fe6:	8d 51 ff             	lea    -0x1(%rcx),%edx	  // edx = 6
  400fe9:	e8 e0 ff ff ff       	callq  400fce <func4> // call func4(i, 0, 6)
  400fee:	01 c0                	add    %eax,%eax    // return *= 2
  400ff0:	eb 15                	jmp    401007 <func4+0x39>    // jmp to B (return) 

  // A
  400ff2:	b8 00 00 00 00       	mov    $0x0,%eax   // return = 0
  400ff7:	39 f9                	cmp    %edi,%ecx   // rcx - rdi
  400ff9:	7d 0c                	jge    401007 <func4+0x39>  // jump to B (return)
  400ffb:	8d 71 01             	lea    0x1(%rcx),%esi  // esi = rcx + 0x1  (rcx = 8)
...
000000000040100c <phase_4>: // rdi is input, i and j are first two ints
  40100c:	48 83 ec 18          	sub    $0x18,%rsp	// add 3 bytes to rsp
  401010:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx	// arg4 = rsp + 12
  401015:	48 8d 54 24 08       	lea    0x8(%rsp),%rdx   // arg3 = rsp + 8
  40101a:	be cf 25 40 00       	mov    $0x4025cf,%esi	// %d $d
  40101f:	b8 00 00 00 00       	mov    $0x0,%eax	// eax = 0
  401024:	e8 c7 fb ff ff       	callq  400bf0 <__isoc99_sscanf@plt> // set first and second number you input to the stack pointer + 8 and + 12
  401029:	83 f8 02             	cmp    $0x2,%eax	// check if theres 2 args
  40102c:	75 07                	jne    401035 <phase_4+0x29> // jump to explode
  40102e:	83 7c 24 08 0e       	cmpl   $0xe,0x8(%rsp)	// i - 14
  401033:	76 05                	jbe    40103a <phase_4+0x2e> // jmp if i <= 14
  401035:	e8 00 04 00 00       	callq  40143a <explode_bomb> // explode
  40103a:	ba 0e 00 00 00       	mov    $0xe,%edx  // arg3 = 14
  40103f:	be 00 00 00 00       	mov    $0x0,%esi  // arg2 = 0
  401044:	8b 7c 24 08          	mov    0x8(%rsp),%edi  // arg1 = i
  401048:	e8 81 ff ff ff       	callq  400fce <func4>  // call func4(i, 0, 14)
  40104d:	85 c0                	test   %eax,%eax    // check if ret is 0
  40104f:	75 07                	jne    401058 <phase_4+0x4c> // explode
  401051:	83 7c 24 0c 00       	cmpl   $0x0,0xc(%rsp)	// check if j = 0
...
000000000040145c <read_six_numbers>:
  40145c:	48 83 ec 18          	sub    $0x18,%rsp              // add 18 bytes to stack
  401460:	48 89 f2             	mov    %rsi,%rdx               // mov 2nd arg to 3rd arg
  401463:	48 8d 4e 04          	lea    0x4(%rsi),%rcx          // load address of 2nd arg + 4 bytes to 4th
  401467:	48 8d 46 14          	lea    0x14(%rsi),%rax         // load address of 2nd arg + 14 bytes to return
  40146b:	48 89 44 24 08       	mov    %rax,0x8(%rsp)          // mov return to stackTop + 8
  401470:	48 8d 46 10          	lea    0x10(%rsi),%rax         // load address of 2nd arg + 10 to return
  401474:	48 89 04 24          	mov    %rax,(%rsp)             // mov return to stackTop
  401478:	4c 8d 4e 0c          	lea    0xc(%rsi),%r9           // load address of stackTop + 12 to r9
  40147c:	4c 8d 46 08          	lea    0x8(%rsi),%r8           // load address of stackTop + 8 to r8
  401480:	be c3 25 40 00       	mov    $0x4025c3,%esi          // mov 0x4025c3 to 2nd arg
  401485:	b8 00 00 00 00       	mov    $0x0,%eax               // mov 0x0 to return
